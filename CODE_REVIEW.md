...
| 8 | `history` / `connections` | `8bc167d` perf(history,connections) · `704a84b` test(connections) | `history.record` дописывает строку в конец (JSONL); полная перезапись — только при переливе выше `MAX_ENTRIES × 1.5 = 750`, а `_read_all` всегда режет выдачу до 500, так что `list()`/`get_for_table()` перелива не показывают. `connections.switch`: 4 чтения + 2 записи → 1 + 1, глобальный файл читается только когда тип негде больше взять. Новый `connections_spec.lua` — 10 характеризационных тестов |
