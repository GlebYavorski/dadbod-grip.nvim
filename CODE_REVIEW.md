...
- **Живая проверка MySQL / PostgreSQL / DuckDB / SQL Server отложена до релиза.** В этом
  окружении гоняется только SQLite: `duckdb` и `sqlcmd` не установлены, PG- и MySQL-сервер
  не подняты. Перед релизом стоит поднять остальные (`seed-pg`, `seed-mysql`, `seed-duckdb`
  в `justfile`) и прогнать по ним в первую очередь то, что второй проход правил вслепую:
  - `get_schema_batch` у duckdb (`is_nullable`, объединённый запрос на `duckdb_columns()`/`duckdb_views()`, порядок колонок) и у sqlserver (`data_type` с длиной/точностью) — `5f195f6`, `ad24958`;
